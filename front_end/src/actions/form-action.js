...
export async function query(prevState, formData) {
	const start_s = encodeURIComponent(formData.get('start_s'));
	const end_s = encodeURIComponent(formData.get('end_s'));
    const date = formData.get('date');
    

	//检查输入合法性
	if (start_s === '' || end_s === '' || date === '') {
	...
	//查询
	const url_direct = `http://127.0.0.1:8000/trains_direct?start_s=${start_s}&end_s=${end_s}&train_date=${date}`;
	const url_change = `http://127.0.0.1:8000/trains_change?start_s=${start_s}&end_s=${end_s}&train_date=${date}`;
	...
	revalidatePath('/');
	return { finish: prevState.finish + 1};
}
